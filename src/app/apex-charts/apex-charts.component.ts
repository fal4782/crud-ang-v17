...
export class ApexChartsComponent {
  ...
  constructor(private apexDataService: ApexDataService, private cdr: ChangeDetectorRef) {}
  ...
  private loadChartData(): void {
    this.apexDataService.getChartData().subscribe(data => {
  ...
  private initializeChart(data: any): void {
    ...
    this.chartOptions = {
      ...
      chart: {
        ...
        type: 'pie',
        events: {
          dataPointSelection: (event:any, chartContext:any, config:any) => this.handleDataPointSelection(config),
        },
      },
      theme: {
        monochrome: {
...
//   drilldown logic starts
  ...
  private handleDataPointSelection(config: any): void {
    if (!this.isDrilldown) {
        ...
        const selectedDrilldown = this.drilldownData.find((d: any) => d.id === selectedId);

        if (selectedDrilldown) {
        this.updateChart(selectedDrilldown);
        this.currentDrilldownLabel = this.originalSeries.data[config.dataPointIndex].label;
  ...
  private updateChart(selectedDrilldown: any): void {
    ...
    const originalItem = this.originalSeries.data.find((item: any) => item.id === selectedDrilldown.id);
    ...
    }, true); // The 'true' parameter enables animation for the options update
